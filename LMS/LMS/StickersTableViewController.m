...
/*
	...
	*/
	...
	NSLog(@"%s | %@", __PRETTY_FUNCTION__, sticker.color);
	
	cell.colorView.backgroundColor = [UIColor colorFromHexString:sticker.color];
...
- (LMSSticker *)stickerAtIndexPath:(NSIndexPath *)indexPath {
	LMSSticker *sticker = nil;
/*
	if (indexPath.section == 0 && [self.myPhoneStickerRecordList count] > 0)
		stickerRecord = [self.myPhoneStickerRecordList objectAtIndex:indexPath.row];
