...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
	...
	History *history = [self.historyList objectAtIndex:indexPath.row];
	
	NSString *historyText = [NSString stringWithFormat:@"%@ was %@", [history.subject capitalizedString], history.operation];

	dispatch_async(dispatch_get_main_queue(), ^{
		cell.historyLabel.text = historyText;
		cell.dateLabel.text = [history.updatedAt distanceOfTimeInWords];//[ConventionTools getDiffTimeInStringFromDate:historyRecord.updatedAt];
    });
    return cell;
}
