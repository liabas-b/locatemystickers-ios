...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    switch (indexPath.row) {
		...
		{
			NSString *text = [self.sticker.lastLocation stringByReplacingOccurrencesOfString:@"," withString:@"\n"];
			...
			NSLog(@"%s | self.sticker.lastLocation: %@", __PRETTY_FUNCTION__, self.sticker.lastLocation);
        ...
        case 4:
        {
			NSString *text = self.sticker.text;
...
- (void)setupView {
	...
	self.colorView.backgroundColor = [UIColor colorFromHexString:self.sticker.color];
}
...
//		NSString *locatedAt = [[placeMark.addressDictionary valueForKey:@"FormattedAddressLines"] componentsJoinedByString:@", "];
		...
		[self.mapView loadSticker:self.sticker];
