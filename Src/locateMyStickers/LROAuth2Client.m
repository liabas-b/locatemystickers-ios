...
- (NSURLRequest *)userAuthorizationRequestWithParameters:(NSDictionary *)additionalParameters;
{
  ...
  if (additionalParameters) {
	...
	//INFO: coreection bug from the web service
	NSString *fullURLString = [NSString stringWithFormat:@"%@&scope=&response_type=code&", [[self.userURL absoluteString] stringByAppendingFormat:@"?%@", [params stringWithFormEncodedComponents]]];
...
- (void)handleCompletionForAuthorizationRequestOperation:(LRURLRequestOperation *)operation
{
  ...
  if (response.statusCode == 200) {
    ...
    if ([authData objectForKey:@"access_token"] == nil) {
		//TODO: do not get the access code --> error
#ifdef DEBUG
		NSLog(@"ERROR access_token: %@", authData);
#endif
...
- (void)authorizeUsingWebView:(UIWebView *)webView;
{
...
#ifdef DEBUG
		NSLog(@"cleaning cookie from: %@", [cookie domain]);
#endif
	...
	//NSDictionary *additionalParameters = [[NSDictionary alloc] initWithObjectsAndKeys:@"code", @"reponse_type", @"", @"scope", nil];
