 ...
 * distributed under the License is distributed on an "AS IS" BASIS,DFF
...
@interface ScanWidgetController ()
...
@implementation ScanWidgetController
...
- (id)initWithDelegate:(id<ScanProtocol>)scanDelegate showCancel:(BOOL)shouldShowCancel OneDMode:(BOOL)shouldUseoOneDMode {
	self = [super init];
	if (self) {
		...
		beepSound = -1;
		decoding = NO;
		ScanOverlayView *theOverLayView = [[ScanOverlayView alloc] initWithFrame:CGRectMake(0, 0, self.view.frame.size.width, self.view.frame.size.height)
...
- (void)editingDoneWithValue:(NSString *)resultValue
{
    NSLog(@"result: %@", resultValue);
	
	[delegate zxingController:self didScanResult:resultValue manualMode:YES];
}


- (void)startCloseTimer {
	NSLog(@"restartCloseTimer");
	if( closeTimer != nil )
		[closeTimer invalidate];
	closeTimer = [NSTimer scheduledTimerWithTimeInterval:readingTimeOut
												  target:self
												selector:@selector(handleCloseTimer:)
												userInfo:nil repeats:NO];
	[[NSRunLoop mainRunLoop] addTimer:closeTimer forMode:NSDefaultRunLoopMode];
}

- (void)handleCloseTimer:(NSTimer*)theTimer {
	NSLog(@"handleCloseTimer");
	[self cancelled];
}
...
- (void)torch {
	static BOOL torchStatus;
	[self setTorch:!torchStatus];
...
//INFO: DecoderDelegate methods

- (void)decoder:(Decoder *)decoder willDecodeImage:(UIImage *)image usingSubset:(UIImage *)subset{
}

- (void)decoder:(Decoder *)decoder decodingImage:(UIImage *)image
...
- (void)presentResultPoints:(NSArray *)resultPoints
                ...
                usingSubset:(UIImage *)subset {
	//INFO: simply add the points to the image view --> WARNING: test !!!
...
- (void)decoder:(Decoder *)decoder didDecodeImage:(UIImage *)image usingSubset:(UIImage *)subset withResult:(TwoDDecoderResult *)twoDResult {
	...
	//INFO now, in a selector, call the delegate to give this overlay time to show the points
...
- (void)notifyDelegate:(id)text {
	...
	[delegate zxingController:self didScanResult:text manualMode:NO];
}
